...
function MODEL:selector(prop, value)
  ...
  if self:page():hasSelection() then
    ...
    if prop == "textsize" or has_text(self:page()) then
      self:autoRunLatex()
    end
  end
...
function MODEL:action_export_png()
  ...
  if not dir then dir = prefs.save_as_directory end
  local s, f =
    ipeui.fileDialog(self.ui:win(), "save", "Export to PNG", filter_png, dir)
  if s then
    if config.toolkit ~= "cocoa" and ipe.fileExists(s) then
      local b = messageBox(self.ui:win(), "question",
			   "File already exists!",
...
function MODEL:action_export_eps()
  ...
  if not dir then dir = prefs.save_as_directory end
  local s, f =
    ipeui.fileDialog(self.ui:win(), "save", "Export to EPS", filter_eps, dir)
  if s then
    if config.toolkit ~= "cocoa" and ipe.fileExists(s) then
      local b = messageBox(self.ui:win(), "question",
			   "File already exists!",
...
function MODEL:wheel_zoom(delta, precise)
  ...
  local base = prefs.wheel_zoom_factor
  if not precise then base = prefs.wheel_other_factor end
  local nzoom = zoom * base ^ delta
  if nzoom > prefs.max_zoom then nzoom = prefs.max_zoom end
  if nzoom < prefs.min_zoom then nzoom = prefs.min_zoom end
  self.ui:setZoom(nzoom)
...
function MODEL:saction_set_link_url()
  local p = self:page()
  local prim = p:primarySelection()
  local obj = p[prim]
  assert(obj:type() == "group")
  local url = self:getString("Enter link destination URL", nil, obj:text())
  if not url then return end
  if url:match("^%s*$)") then url = "" end
  local t = { label="set link URL",
	      pno=self.pno,
	      vno=self.vno,
	      primary=prim,
	      original=obj:clone(),
  ...
  t.redo = function (t, doc)
	     doc[t.pno][t.primary]:setText(t.url)
	   end
  self:register(t)
end
